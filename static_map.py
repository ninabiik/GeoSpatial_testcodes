...
st.set_page_config(layout="wide", page_title="Traffic Visualisation", page_icon="🚦")
...
def get_color(row):
    ...
    min_count = filtered_df['TRAFFIC_COUNT'].min()
    max_count = filtered_df['TRAFFIC_COUNT'].max()
...
def main():
    ...
    # Sidebar - Year and Period selection
    ...
    year = st.sidebar.slider('Select Year', min_value=year_min, max_value=year_max)

    filtered_df = filtered_df[filtered_df['YEAR'] == str(year)]

    max_range = filtered_df["TRAFFIC_COUNT"].max()
    filtered_df['COLOR_COLUMN'] = filtered_df.apply(get_color, axis=1)

    # Create a layout with two columns
    col1, cols_space, col2 = st.columns([4, 0.5, 2])

    # Display map using Pydeck HexagonLayer in the first column
    with col1:
        st.subheader("Exploring Spatial Traffic Patterns Across Sydney")
    ...
    # Display bar chart on the right side in the second column
    with col2:
